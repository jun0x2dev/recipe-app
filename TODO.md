...
- [x] 구글 iOS 로그인 expo-auth-session 기반 구현
- [x] 구글 iOS Client ID 추가 및 개발 빌드 검증
- [ ] 앱 내 계정 삭제 화면 구현
- [ ] 자체 회원가입 화면 후순위 전환
- [x] 모바일 UI 디자인 전면 교체
- [ ] 이미지 첨부 기능 검토
...
- [x] 구글 iOS Client ID 백엔드 audience 허용 추가
- [ ] 소셜 계정 연결 정책 구현
- [ ] 앱 내 계정 삭제 API 구현
- [x] Gradle Wrapper 생성
...
- [x] 조회수 증가 로직 구현 (타인 레시피 조회 시)
- [x] 좋아요 토글 API 구현 (recipe_likes 테이블, 엔티티, 서비스, 컨트롤러)
- [x] 좋아요 상태 목록/상세 API 응답에 포함
- [x] 모바일 좋아요 버튼 UI 추가 (상세 + 목록)
- [x] 좋아요한 레시피 목록 API 구현 (GET /api/v1/recipes/liked)
- [x] 하단 탭 설정 → 메뉴로 전환 (메뉴 > 좋아요한 레시피, 설정)
- [x] 좋아요한 레시피 목록 화면 구현
- [x] 메뉴 화면에 로그인 제공자/이메일 표시
- [x] 로그인 화면에 최근 로그인 방법 표시 (expo-secure-store 영속 저장)
- [ ] 둘러보기 화면 정렬 기능 추가 (최신순, 좋아요순, 조회순 등)
- [ ] 공유 기능 설계
...
- [x] 유튜브 쇼츠 입력 방식 검토
- [x] STT 임시 비활성화 - CPU 처리 속도 문제로 오디오 다운로드/STT 건너뜀
...
- 레시피 임시 저장 기능
- [ ] 레시피 보고 음식 만들기 시작 기능 - 레시피의 스텝을 읽어주는 기능 - 1 단계 읽어주고, [다시 듣기], [다음 단계] 버튼 구현하기(기여운 캐릭터가 움직임!)
- [ ] 어플 이름 생각하기 (후보: 냠로그, )
